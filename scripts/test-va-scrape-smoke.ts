...
import {
  loadVaRowsFromCsvPath,
  runVaGdcourtsFlow,
  type VaSearchRow,
} from "./va-gdcourts-scrape.playwright"
...
async function main() {
  ...
  const headed = process.env.PLAYWRIGHT_HEADED === "1" || process.env.VA_SCRAPE_HEADED === "1"
  console.log(`[smoke] rows=${rows.length} headless=${!headed} (set PLAYWRIGHT_HEADED=1 to watch)`)
